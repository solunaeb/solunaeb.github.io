...
self.addEventListener('push', (e) => {
  ...
  e.waitUntil((async () => {
    // 앱이 같은 알림을 이미 띄웠으면 두 번 울리지 않기
    if (data.tag && (await self.registration.getNotifications({ tag: data.tag })).length) return;
    await self.registration.showNotification(data.title || '널 사랑할고양', {
      body: data.body || '', tag: data.tag, renotify: true, icon: './icons/icon-192.png', badge: './icons/badge-96.png', data: { url: './' },
    });
  })());
});
